...
function desenharProdutoNoCarrinho(idProduto) {
  ...
  const cartaoProdutoCarrinho = `<button id="remover-item-${
    ...
    <div class="flex text-slate-950 items-center absolute bottom-1 right-2 text-lg">
      <button id="decrementar-produto-${
        produto.id
      }" class="bg-slate-500 hover:bg-slate-800 rounded-full p-1">-</button>
      <p id="quantidade-${produto.id}" class="ml-2">${
    idsProdutoCarrinhoComQuantidade[produto.id]
  }</p>
      <button id="incrementar-produto-${
        produto.id
      }" class="ml-2 bg-slate-500 hover:bg-slate-800 rounded-full p-1">+</button>
    </div>`;
